...
class InsertHabitUseCaseTest {
    ...
    @Test
    fun `test Insert Habit Use Case`() = runBlocking {
        ...
        Mockito.`when`(habitsRepository.insertHabit(testHabit)).thenReturn(Unit)


        val insertHabitUseCase = InsertHabitUseCase(habitsRepository, TestCoroutineDispatcher())
        val actualResult = insertHabitUseCase.insertHabit(testHabit)
