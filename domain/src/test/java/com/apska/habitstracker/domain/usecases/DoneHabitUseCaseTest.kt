...
class DoneHabitUseCaseTest {
    ...
    fun `test Done Habit Use Case`() = runBlocking {
        ...
        Mockito.`when`(habitsRepository.doneHabit(testHabit)).thenReturn(true)


        val doneHabitUseCase = DoneHabitUseCase(habitsRepository, TestCoroutineDispatcher())
        val actualResult = doneHabitUseCase.doneHabit(testHabit)
