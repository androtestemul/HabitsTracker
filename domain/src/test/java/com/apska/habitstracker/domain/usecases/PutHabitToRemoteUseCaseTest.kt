...
class PutHabitToRemoteUseCaseTest {

    private val habitsRepository = mock<HabitsRepository>()

    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `test Put Habit To Remote Use Case`() = runBlocking {
        ...
        Mockito.`when`(habitsRepository.putHabitToRemote(testHabit)).thenReturn(HABIT_NEW_UID)

        val putHabitToRemoteUseCaseTest = PutHabitToRemoteUseCase(habitsRepository, TestCoroutineDispatcher())
        val actualResult = putHabitToRemoteUseCaseTest.putHabitToRemote(testHabit)

        val expectedResult = HABIT_NEW_UID
    ...
    companion object {
        private const val HABIT_NEW_UID = "new_uid"
