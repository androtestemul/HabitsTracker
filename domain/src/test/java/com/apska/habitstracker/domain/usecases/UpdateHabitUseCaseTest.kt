...
class UpdateHabitUseCaseTest {
    ...
    @Test
    fun `test Update Habit Use Case`() = runBlocking {
        ...
        Mockito.`when`(habitsRepository.updateHabit(testHabit)).thenReturn(Unit)


        val updateHabitUseCase = UpdateHabitUseCase(habitsRepository, TestCoroutineDispatcher())
        val actualResult = updateHabitUseCase.updateHabit(testHabit)
